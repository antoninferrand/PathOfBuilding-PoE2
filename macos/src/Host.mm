    ...
    while (running) {
        ...
        if (renderer) {
            ...
            if (screenshotRequested) {
                ...
                fs::path screenshotDir = fs::path(applicationSupportPath()) / "Path of Building (PoE2)" / "Screenshots";
                std::error_code ec;
                fs::create_directories(screenshotDir, ec);
                SDL_Surface* surface = ec ? nullptr : SDL_RenderReadPixels(renderer, nullptr);
                if (surface) {
                    auto now = std::chrono::duration_cast<std::chrono::milliseconds>(
                        std::chrono::system_clock::now().time_since_epoch()).count();
                    fs::path screenshot = screenshotDir / ("screenshot-" + std::to_string(now) + ".bmp");
                    if (!SDL_SaveBMP(surface, screenshot.string().c_str())) {
                        std::fprintf(stderr, "Screenshot failed: %s\n", SDL_GetError());
                    }
                    SDL_DestroySurface(surface);
                } else {
                    std::fprintf(stderr, "Screenshot failed: %s\n", ec ? ec.message().c_str() : SDL_GetError());
                }
            }
            SDL_SetRenderViewport(renderer, nullptr);
...
void Host::registerApi() {
    ...
    registerFunction(L, "GetCloudProvider", l_GetCloudProvider);
    ...
    registerFunction(L, "ConPrintTable", l_ConPrintTable);
    registerFunction(L, "ConExecute", l_ConExecute);
    registerFunction(L, "ConClear", l_ConClear);
    registerFunction(L, "TakeScreenshot", l_TakeScreenshot);
...
int Host::l_GetCloudProvider(lua_State* L) {
    lua_pushliteral(L, "Unknown");
    lua_pushnil(L);
    lua_pushliteral(L, "Cloud provider detection is unavailable on macOS");
    return 3;
}
...
int Host::l_ConPrintTable(lua_State* L) {
...
int Host::l_TakeScreenshot(lua_State*) {
    if (current) {
        current->screenshotRequested = true;
    }
    return 0;
}
